...
import type { AvailableProvider, UserOAuthBinding } from '../../api/userOAuthApi'
import {
  authorizeProvider,
  generateOAuthState,
...
async function handleBindProvider(provider: OAuthProviderName) {
  try {
...
async function handleBindingRefresh(binding: UserOAuthBinding) {
  try {
...
async function handleBindingUnbind(binding: UserOAuthBinding) {
  try {
...
async function handleBindingUpdated() {
  await loadData()
}

// OAuth flow handlers
function handleOAuthSuccess(result: any) {
...
function getProviderDescription(provider: OAuthProviderName): string {
  const descriptions = {
...
function getProviderDisplayName(provider: OAuthProviderName): string {
  const names = {
    dingtalk: '钉钉',
    github: 'GitHub',
    gitee: 'Gitee',
  ...
  <div class="personal-oauth-bindings">
    ...
    <div v-else class="content-container">
      ...
      <el-card class="security-card" shadow="never">
        ...
        <el-row :gutter="24">
          <el-col v-for="tip in securityTips" :key="tip.title" :xs="24" :sm="12" :lg="6">
            <div class="tip-item">
              <div class="tip-icon">
                <el-icon :size="20">
    ...
    >
      <div class="oauth-dialog-content">
        <div class="oauth-loading-indicator">
          <el-icon class="is-loading oauth-spinner">
            <Loading />
          </el-icon>
          <p>正在跳转到授权页面...</p>
        </div>
